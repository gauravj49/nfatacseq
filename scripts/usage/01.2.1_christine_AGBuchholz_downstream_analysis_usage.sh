...
# From the cutoff analysis, we identified a cutoff of pvalue=0.004
pvalueDir="0_004" ; # pvalue=0.004 or pscore ~ 2.40
pvalue="${pvalueDir//_/.}" 
customPeaksDir="${analysisDir}/customPeaks/mergedLibrary"; mkdir -p ${customPeaksDir}
...
# 3.2) HMMRATAC Peak calling
customPeaksDir="${analysisDir}/customPeaks/mergedLibrary"; mkdir -p ${customPeaksDir}
peaksOutDir=${customPeaksDir}/hmmratac; mkdir -p ${peaksOutDir}
genomeInfoFile=${customPeaksDir}/genome.info

# 3.2.1) Make genome information (chromosome sizes) from the BAM file to get a genome.info file:
# Get first file from ls command ls -1 -t ${bamDir}/*.bam | head -1
samtools view -H $(ls -1 -t ${bamDir}/*.bam | head -1)| perl -ne 'if(/^@SQ.*?SN:(\w+)\s+LN:(\d+)/){print $1,"\t",$2,"\n"}' > ${genomeInfoFile}
for b in ${bamDir}/*.bam;
do
  bname=$(basename ${b} .mLb.clN.sorted.bam)
  dbname=${peaksOutDir}/${bname}
  echo "- Running HMMRATAC for ${bname}"
  ...
  # java -jar /home/rad/packages/hmmratac/HMMRATAC_V1.2.10_exe.jar -b ${b} -i ${b}.bai -g ${genomeInfoFile} -o ${bname} --bedgraph true -q 20
  ...
  awk -v OFS="\t" '$13>=10 {print}' ${dbname}_peaks.gappedPeak > ${dbname}.filteredPeaks.gappedPeak

  # To filter the summit file by the same threshold:
  awk -v OFS="\t" '$5>=10 {print}' ${dbname}_summits.bed > ${dbname}.filteredSummits.bed
  echo ""
done
