...
# 29th Oct 2020
# Generate annotation for filtered consensus regions
# 3.2.6.2) Get the annotation file for the DARs
python -c "import sys; import pandas as pd; import datatable as dt; input_file=sys.argv[1]; input_bed=sys.argv[2]; outtxt_file=sys.argv[3]; infileDT = dt.fread(input_file, sep='\t', header=True, nthreads=16); bedDT = dt.fread(input_bed, sep='\t', header=True, nthreads=16); infileDT = dt.fread(input_file, sep='\t', header=True, nthreads=16); bedDT = dt.fread(input_bed, sep='\t', header=True, nthreads=16); infileDF = infileDT.to_pandas(); infileDF.set_index('PeakID', inplace=True); bedDF = bedDT.to_pandas(); bedDF.set_index('PeakID', inplace=True); infileDF = infileDF[infileDF.index.isin(bedDF.index)]; infileDF.reset_index(level=0, inplace=True); infileDFColumns = infileDF.columns.tolist(); infileDFColumns.insert(3,infileDFColumns.pop(0)); infileDF = infileDF[infileDFColumns]; infileDF.to_csv(outtxt_file, header=True, index=False, sep='\t', float_format='%.0f');" /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/fignColCancer_consensus_peaks_annotation.txt /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/1033_colon_filtered_CRs.txt /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/1033_colon_filtered_CRs_annotation.txt
python -c "import sys; import pandas as pd; import datatable as dt; input_file=sys.argv[1]; input_bed=sys.argv[2]; outtxt_file=sys.argv[3]; infileDT = dt.fread(input_file, sep='\t', header=True, nthreads=16); bedDT = dt.fread(input_bed, sep='\t', header=True, nthreads=16); infileDT = dt.fread(input_file, sep='\t', header=True, nthreads=16); bedDT = dt.fread(input_bed, sep='\t', header=True, nthreads=16); infileDF = infileDT.to_pandas(); infileDF.set_index('PeakID', inplace=True); bedDF = bedDT.to_pandas(); bedDF.set_index('PeakID', inplace=True); infileDF = infileDF[infileDF.index.isin(bedDF.index)]; infileDF.reset_index(level=0, inplace=True); infileDFColumns = infileDF.columns.tolist(); infileDFColumns.insert(3,infileDFColumns.pop(0)); infileDF = infileDF[infileDFColumns]; infileDF.to_csv(outtxt_file, header=True, index=False, sep='\t', float_format='%.0f');" /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/fignColCancer_consensus_peaks_annotation.txt /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/1033_duo_filtered_CRs.txt /media/rad/HDD1/atacseq/miguel/fignColCancer/analysis/analysis_q0_01/1033_duo_filtered_CRs_annotation.txt